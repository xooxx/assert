...
class DateTimeAssertions
{
    ...
    public static function isDateTime($value, $message = '')
    {
        ...
        if (false === (0 == $errors['warning_count'] && 0 == $errors['error_count'])) {
            throw new AssertionException($message ? $message : sprintf(self::ASSERT_DATE_TIME, gettype($value)));
        }
    }
    ...
    public static function isAfter($value, $limit, $inclusive = false, $message = '')
    {
        ...
        if (false === $inclusive) {
            if (!(strtotime($value->format('Y-m-d H:i:s')) > strtotime($limit->format('Y-m-d H:i:s')))) {
                throw new AssertionException($message ? $message : sprintf(self::ASSERT_IS_AFTER, $limit->format('Y-m-d H:i:s')));
            }
        }
        if (!(strtotime($value->format('Y-m-d H:i:s')) >= strtotime($limit->format('Y-m-d H:i:s')))) {
            throw new AssertionException($message ? $message : sprintf(self::ASSERT_IS_AFTER, $limit->format('Y-m-d H:i:s')));
    ...
    public static function isBefore($value, $limit, $inclusive = false, $message = '')
    {
        ...
        if (false === $inclusive) {
            if (!(strtotime($value->format('Y-m-d H:i:s')) < strtotime($limit->format('Y-m-d H:i:s')))) {
                throw new AssertionException($message ? $message : sprintf(self::ASSERT_IS_BEFORE, $limit->format('Y-m-d H:i:s')));
            }
        }
        if (!(strtotime($value->format('Y-m-d H:i:s')) <= strtotime($limit->format('Y-m-d H:i:s')))) {
            throw new AssertionException($message ? $message : sprintf(self::ASSERT_IS_BEFORE, $limit->format('Y-m-d H:i:s')));
    ...
    public static function isDateRange($value, $minDate, $maxDate, $inclusive = false, $message = '')
    {
        ...
        if (!$inclusive && !($value > $minDate && $value < $maxDate)) {
            ...
            throw new AssertionException($message ? $message : sprintf(self::ASSERT_IS_BETWEEN, $minDate, $maxDate));
        }
        if ($inclusive && !($value >= $minDate && $value <= $maxDate)) {
            $minDate = $minDate->format('Y-m-d H:i:s');
            $maxDate = $maxDate->format('Y-m-d H:i:s');
    ...
    public static function isWeekend($value, $message = '')
    {
        ...
        if ('0' != $value->format('w') && '6' != $value->format('w')) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_WEEKEND);
        }
    }
    ...
    public static function isWeekday($value, $message = '')
    {
        ...
        if ($value->format('w') == 0 || $value->format('w') == 6) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_WEEKDAY);
        }
    }
    ...
    public static function isMonday($value, $message = '')
    {
        $value = self::convertToDateTime($value);
        if (!('1' == $value->format('w'))) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_MONDAY);
        }
    }
    ...
    public static function isTuesday($value, $message = '')
    {
        $value = self::convertToDateTime($value);
        if (!('2' == $value->format('w'))) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_TUESDAY);
        }
    }
    ...
    public static function isWednesday($value, $message = '')
    {
        $value = self::convertToDateTime($value);
        if (!('3' == $value->format('w'))) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_WEDNESDAY);
        }
    }
    ...
    public static function isThursday($value, $message = '')
    {
        $value = self::convertToDateTime($value);
        if (!('4' == $value->format('w'))) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_THURSDAY);
        }
    }
    ...
    public static function isFriday($value, $message = '')
    {
        $value = self::convertToDateTime($value);
        if (!('5' == $value->format('w'))) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_FRIDAY);
        }
    }
    ...
    public static function isSaturday($value, $message = '')
    {
        $value = self::convertToDateTime($value);
        if (!('6' == $value->format('w'))) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_SATURDAY);
        }
    }
    ...
    public static function isSunday($value, $message = '')
    {
        $value = self::convertToDateTime($value);
        if (!('0' == $value->format('w'))) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_SUNDAY);
        }
    }
    ...
    public static function isToday($value, $message = '')
    {
        ...
        if (!($date->format('Y-m-d') === $value->format('Y-m-d'))) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_TODAY);
        }
    }
    ...
    public static function isYesterday($value, $message = '')
    {
        ...
        if (!($date->format('Y-m-d') === $value->format('Y-m-d'))) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_YESTERDAY);
        }
    }
    ...
    public static function isTomorrow($value, $message = '')
    {
        ...
        $date = new DateTime('now + 1 day');
        if (!($date->format('Y-m-d') === $value->format('Y-m-d'))) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_TOMORROW);
        }
    }
    ...
    public static function isLeapYear($value, $message = '')
    {
        $value = self::convertToDateTime($value);
        if (!('1' == $value->format('L'))) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_LEAP_YEAR);
        }
    }
    ...
    public static function isMorning($value, $message = '')
    {
        ...
        if (!($date >= strtotime($value->format('06:00:00')) && $date <= strtotime($value->format('11:59:59')))) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_MORNING);
        }
    }
    ...
    public static function isAfternoon($value, $message = '')
    {
        ...
        if (!($date >= strtotime($value->format('12:00:00')) && $date <= strtotime($value->format('17:59:59')))) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_AFTERNOON);
        }
    }
    ...
    public static function isEvening($value, $message = '')
    {
        ...
        if (!($date >= strtotime($value->format('18:00:00')) && $date <= strtotime($value->format('23:59:59')))) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_EVENING);
        }
    }
    ...
    public static function isNight($value, $message = '')
    {
        ...
        if (!($date >= strtotime($value->format('00:00:00')) && $date <= strtotime($value->format('05:59:59')))) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_NIGHT);
        }
    }
    ...
    public static function isFutureDate($value, $message = '')
    {
        ...
        if (false === $value > new DateTime()) {
            throw new AssertionException($message ? $message : self::ASSERT_FUTURE_DATE);
        }
    }
    ...
    public static function isPastDate($value, $message = '')
    {
        $value = self::convertToDateTime($value);
        if (false === $value < new DateTime()) {
            throw new AssertionException($message ? $message : self::ASSERT_PAST_DATE);
        }
    }
    ...
    public static function isInNextWeek($value, $message = '')
    {
        ...
        self::isInWeek($value, $nextWeek, $message ? $message : sprintf(self::ASSERT_IS_NEXT_WEEK, $nextWeek));
    }
    ...
    public static function isInWeek($value, $weekNumber, $message = '')
    {
        ...
        if (false === ($weekValue == $weekNumber)) {
            throw new AssertionException($message ? $message : sprintf(self::ASSERT_IS_IN_WEEK, $weekNumber));
        }
    }
    ...
    public static function isInLastWeek($value, $message = '')
    {
        ...
        self::isInWeek($value, $lastWeek, $message ? $message : sprintf(self::ASSERT_IS_LAST_WEEK, $lastWeek));
    }
    ...
    public static function isInLastMonth($value, $message = '')
    {
        ...
        self::isInMonth($value, $past->format('m'), $message ? $message : sprintf(self::ASSERT_IS_IN_PAST_MONTH, $past->format('F')));
    }
    ...
    public static function isInMonth($value, $monthNumber, $message = '')
    {
        $value = self::convertToDateTime($value);
        $currentYear = date('Y');
        if ($currentYear != $value->format('Y') || $monthNumber != $value->format('n')) {
            throw new AssertionException($message ? $message : sprintf(self::ASSERT_IS_IN_MONTH, (new DateTime('1970-' . $monthNumber . '-01'))->format('F')));
        }
    }
    ...
    public static function isInNextMonth($value, $message = '')
    {
        ...
        self::isInMonth($value, $next->format('m'), $message ? $message : sprintf(self::ASSERT_IS_IN_NEXT_MONTH, $next->format('F')));
    }
    ...
    public static function isInLastYear($value, $message = '')
    {
        ...
        self::isInYear($value, $pastYear, $message ? $message : sprintf(self::ASSERT_IS_IN_PAST_YEAR, $pastYear));
    }
    ...
    public static function isInYear($value, $yearNumber, $message = '')
    {
        ...
        if ($value->format('Y') != $yearNumber) {
            throw new AssertionException($message ? $message : sprintf(self::ASSERT_IS_IN_YEAR, $yearNumber));
        }
    }
    ...
    public static function isInNextYear($value, $message = '')
    {
        ...
        self::isInYear($value, $next, $message ? $message : sprintf(self::ASSERT_IS_IN_NEXT_YEAR, $next));
    }
    ...
    public static function isFirstHalfOfYear($value, $message = '')
    {
        ...
        if (1 == $half) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_SECOND_HALF_YEAR);
        }
    }
    ...
    public static function isTrimesterOfYear($value, $trimester, $message = '')
    {
        ...
        if ($currentTrimester != $trimester - 1) {
            throw new AssertionException($message ? $message : sprintf(self::ASSERT_IS_TRIMESTER, $trimester));
        }
    }
    ...
    public static function isQuarterOfYear($value, $quarter, $message = '')
    {
        ...
        if ($currentQuarter != $quarter) {
            throw new AssertionException($message ? $message : sprintf(self::ASSERT_IS_QUARTER, $quarter));
        }
    }
