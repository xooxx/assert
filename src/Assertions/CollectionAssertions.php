...
class CollectionAssertions
{
    ...
    {
        $result = is_array($value) || is_object($value) && $value instanceof \ArrayObject || is_object($value) && $value instanceof \SplFixedArray;
        if (false === $result) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_ARRAY);
        }
    }
    /**
     * @param $value
    ...
    public static function each($value, callable $valueValidator, callable $keyValidator = null, $message = '')
    {
        foreach ($value as $key => $data) {
            try {
                ...
                throw new AssertionException($message ? $message : $e->getMessage());
            }
        }
    }
    /**
    ...
    public static function hasKeyFormat($value, callable $keyValidator, $message = '')
    {
        ...
        foreach ($arrayKeys as $key) {
            try {
                ...
                throw new AssertionException($message ? $message : self::ASSERT_KEY_FORMAT);
            }
        }
    }
    /**
    ...
    public static function endsWith($haystack, $needle, $strict = false, $message = '')
    {
        ...
        if (false === $strict) {
            if ($last != $needle) {
                throw new AssertionException($message ? $message : self::ASSERT_ENDS_WITH);
            }
            return;
        }
        if ($last !== $needle) {
    ...
    public static function hasKey($value, $keyName, $message = '')
    {
        if (false === array_key_exists($keyName, $value)) {
            throw new AssertionException($message ? $message : sprintf(self::ASSERT_HAS_KEY, $keyName));
        }
    }
    /**
     * @param $value
    ...
    public static function hasLength($value, $length, $message = '')
    {
        ...
        if (false === (count($value) === $length)) {
            throw new AssertionException($message ? $message : sprintf(self::ASSERT_HAS_LENGTH, $length));
        }
    }
    /**
     * @param $value
    ...
    public static function isNotEmpty($value, $message = '')
    {
        if (false === count($value) > 0) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_NOT_EMPTY);
        }
    }
    /**
     * @param $haystack
    ...
    public static function startsWith($haystack, $needle, $strict = false, $message = '')
    {
        ...
        if (false === $strict) {
            if ($first != $needle) {
                throw new AssertionException($message ? $message : self::ASSERT_STARTS_WITH);
            }
            return;
        }
        if ($first !== $needle) {
