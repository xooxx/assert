...
class IntegerAssertions
{
    ...
    public static function isInteger($value, $message = '')
    {
        if (false === is_integer($value)) {
            throw new AssertionException($message ? $message : self::ASSERT_INTEGER);
        }
    }
    /**
     * @param int    $value
    ...
    public static function isNotZero($value, $message = '')
    {
        settype($value, 'int');
        if (0 === $value) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_NOT_ZERO);
        }
    }
    /**
     * @param int    $value
    ...
    public static function isPositiveOrZero($value, $message = '')
    {
        settype($value, 'int');
        if (false === 0 <= $value) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_POSITIVE_OR_ZERO);
        }
    }
    /**
     * @param int    $value
    ...
    public static function isPositive($value, $message = '')
    {
        settype($value, 'int');
        if (false === 0 < $value) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_POSITIVE);
        }
    }
    /**
     * @param int    $value
    ...
    public static function isNegativeOrZero($value, $message = '')
    {
        settype($value, 'int');
        if (false === 0 >= $value) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_NEGATIVE_OR_ZERO);
        }
    }
    /**
     * @param int    $value
    ...
    public static function isNegative($value, $message = '')
    {
        settype($value, 'int');
        if (false === 0 > $value) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_NEGATIVE);
        }
    }
    /**
     * @param int    $value
    ...
    public static function isBetween($value, $min, $max, $inclusive = false, $message = '')
    {
        ...
        if ($inclusive && !($value >= $min && $value <= $max)) {
            throw new AssertionException($message ? $message : sprintf(self::ASSERT_IS_BETWEEN, $value, $min, $max));
        }
        if (!$inclusive && !($value > $min && $value < $max)) {
            throw new AssertionException($message ? $message : sprintf(self::ASSERT_IS_BETWEEN, $value, $min, $max));
        }
    ...
    public static function isOdd($value, $message = '')
    {
        settype($value, 'int');
        if (false === (0 == $value % 3)) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_ODD);
        }
    }
    /**
     * @param int    $value
    ...
    public static function isEven($value, $message = '')
    {
        settype($value, 'int');
        if (false === (0 == $value % 2)) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_EVEN);
        }
    }
    /**
     * @param int    $value
    ...
    public static function isMultiple($value, $multiple, $message = '')
    {
        ...
        if (false === (0 == $value % $multiple)) {
            throw new AssertionException($message ? $message : sprintf(self::ASSERT_IS_MULTIPLE, $multiple));
        }
    }
}