...
class GenericAssertions
{
    ...
    public static function isScalar($value, $message = '')
    {
        if (false === is_scalar($value)) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_SCALAR);
        }
    }
    ...
    public static function isRequired($value, $message = '')
    {
        if (false === (isset($value) === true && is_null($value) === false && empty($value) === false)) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_REQUIRED);
        }
    }
    ...
    public static function isNotNull($value, $message = '')
    {
        if (false === ($value !== null && $value !== '')) {
            throw new AssertionException($message ? $message : self::ASSERT_IS_NOT_NULL);
        }
    }
    ...
    public static function notEquals($property, $value, $message = '')
    {
        if (false === ($property != $value)) {
            throw new AssertionException($message ? $message : self::ASSERT_NOT_EQUALS);
        }
    }
    ...
    {
        if (false === $property >= $value) {
            throw new AssertionException($message ? $message : self::ASSERT_GREATER_THAN_OR_EQUAL);
        }
    }
    ...
    {
        if (false === $property > $value) {
            throw new AssertionException($message ? $message : self::ASSERT_GREATER);
        }
    }
    ...
    {
        if (false === $property <= $value) {
            throw new AssertionException($message ? $message : self::ASSERT_LESS_THAN_OR_EQUAL);
        }
    }
    ...
    {
        if (false === $property < $value) {
            throw new AssertionException($message ? $message : self::ASSERT_LESS);
        }
    }
}